...
ggplot(match_results_summary, aes(x = factor(result, levels = c("W", "D", "L")), y = number)) +
  geom_bar(stat = "identity", fill = west_ham_sky_blue) +
  geom_text(aes(label = paste0(round(percentage), "%")), vjust = 2, color = west_ham_dark_blue, size = 4) +
  west_ham_theme +
  labs(title = "Match Results in the Period",
       x = "Result",
       y = "Number of Matches",
       fill = "Result")
